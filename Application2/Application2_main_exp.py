...
# Imports for your model, dataset, and parameters (3-input)
...
DEVICE_NAME = "MK_SS_8channels"
# DEVICE_ADDRESS = "F5:45:B3:3B:29:4F"
# DEVICE_ADDRESS = "E0:0F:51:26:9D:48"
DEVICE_ADDRESS = "F1:F5:39:48:E8:D7"
# DEVICE_ADDRESS = "D0:A3:A2:66:C3:48"
# DEVICE_ADDRESS = "C3:42:5C:75:AB:6E"
...
BLE_CLIENT = None          # shared BleakClient across pre-task and Task1
REST_CLASS_INDEX = 0       # rest class index used in cursor control
REST_PROB_THRESHOLD = 0.80  # change this value as needed
PINCH_PROB_THRESHOLD = 0.85  # change this value as needed for pinch
...
CSV_LOG_DIR = "realtime_csv_logs_0406_ws120_until_acc85_pinch_anywhere_R0.8_P0.85"
...
def stop_sensor_csv_logger() -> None:
    """Stop the background CSV logger and flush remaining rows."""
...
###############################################################################
# Global objects for dashboard (2nd window)
###############################################################################
DASH_WIDTH = 500
DASH_HEIGHT = 350
DISPLAY_MIN = 0.0
DISPLAY_MAX = 14000.0
DASH_LABEL_FONT = ("Helvetica", 14)

dash_win = None
dash_canvas = None


# Channel buffer for the live graph
CHANNEL_BUFFERS = {
    1: deque(maxlen=600),  # CH1
    2: deque(maxlen=600),  # CH2
    # 3: deque(maxlen=600),  # CH3
}
...
GRAPH_GEOM = None  # (gx0, gy1, gw, gh) for coordinate mapping

_DASHBOARD_STATIC_READY = False
_DASHBOARD_DYNAMIC_READY = False
_DASH_MINMAX_STATIC_READY = False
_DASH_CHANNEL_LINE_ITEMS = {}
_DASH_GRAPH_POINT_CACHE = {}


###############################################################################
# BLE packet callback
###############################################################################
def ble_notification_callback(sender: int, data: bytearray):
    """
    Parse each BLE packet once, then update the live graph, pre-task dataset,
    ...
    # Live channel graph
    if 1 in CHANNEL_BUFFERS:
        CHANNEL_BUFFERS[1].append(sensors.get("Sensor 1", 0))
    if 2 in CHANNEL_BUFFERS:
        CHANNEL_BUFFERS[2].append(sensors.get("Sensor 2", 0))
    # if 3 in CHANNEL_BUFFERS:
    #     CHANNEL_BUFFERS[3].append(sensors.get("Sensor 3", 0))
...
async def _show_instruction(text, seconds):
    ...
    The Tk update loop keeps both windows refreshing while the overlay is shown.
    """
...
async def precollect_minmax_sequence():
    ...
    channels, compute per-sensor min/max, and keep notifications running for Task1.
    """
    ...
    if BLE_CLIENT is None or not BLE_CLIENT.is_connected:
        ...
        print("[PreCollect] Started notifications (will remain running for Task1).")
    ...
    # Show the final ready screen without stopping notifications.
    await _show_instruction("Get ready", 2)
...
###############################################################################
# Task 1: Packet Reception (BLE)
###############################################################################
async def task1_packet_receive():
    global BLE_CLIENT

    if BLE_CLIENT is not None and BLE_CLIENT.is_connected:
        # Keep the task alive without starting notifications again.
        while not APP_SHUTDOWN.is_set():
            await asyncio.sleep(100000.0)

        # Clean shutdown: stop notifications and disconnect.
        try:
        ...
        # Fallback path when pre-task did not start BLE.
        print(f"[Task1] Connecting to {DEVICE_NAME} ({DEVICE_ADDRESS})...")
        async with BleakClient(DEVICE_ADDRESS) as client:
            BLE_CLIENT = client
            print("[Task1] Connected. Discovering services...")
            services = await client.get_services()
            for svc in services:
                print(f"[Task1] Service: {svc}")
            try:
                await client.start_notify(CHARACTERISTIC_UUID, ble_notification_callback)
                print("[Task1] Started notifications. Receiving data...")
                while not APP_SHUTDOWN.is_set():
                    await asyncio.sleep(1.0)
            except Exception as e:
                print(f"[Task1] Error during BLE notifications: {e}")
            finally:
                # Only stop notifications if Task1 created the session.
                await client.stop_notify(CHARACTERISTIC_UUID)
                print("[Task1] Stopped notifications.")
...
def _task2_window_to_sample(window_buf: np.ndarray,
    ...
    where T = window_size // chunk_size (for window_size = 120 and chunk_size = 10, T = 12).
    """
    # work_buf = normalized (and then sorted per chunk in-place)
    work_buf[:] = window_buf

    # Per-sensor min–max normalization (matches BLEPacketDataset.min_max_normalization())
...
def _show_chrome_popup(duration_ms: int) -> None:
    """Request that chrome.png stays visible until the given deadline."""
...
###############################################################################
# Main window / dashboard UI helpers
###############################################################################
...
def open_dashboard_window():
    """Create the second window and initialize its static/dynamic scene once."""
    global dash_win, dash_canvas

    dash_win = tk.Toplevel(root)
    dash_win.title("Live Monitor")
    ...
    dash_win.protocol("WM_DELETE_WINDOW", _on_dash_close)
    dash_canvas = tk.Canvas(dash_win, width = DASH_WIDTH, height = DASH_HEIGHT, bg = "white")
    dash_canvas.pack()

    _reset_dashboard_scene_cache()
    _init_dashboard_static_scene()
    _init_dashboard_dynamic_scene()


def _reset_dashboard_scene_cache():
    """Reset cached dashboard item ids and geometry."""
    global GRAPH_GEOM
    global _DASHBOARD_STATIC_READY, _DASHBOARD_DYNAMIC_READY, _DASH_MINMAX_STATIC_READY
    global _DASH_CHANNEL_LINE_ITEMS, _DASH_GRAPH_POINT_CACHE
...
def _init_dashboard_static_scene():
    """
    Create static dashboard items once.

    Static items:
      - vertical axis
      - 0 / 7k / 14k axis labels
      - CH1 / CH2 / CH3 color indicators
    """
    global GRAPH_GEOM, _DASHBOARD_STATIC_READY

    if dash_canvas is None or _DASHBOARD_STATIC_READY:
        return

    graph_bbox = (10, 10, DASH_WIDTH - 10, DASH_HEIGHT - 10)

    margin_left, margin_right, margin_top, margin_bottom = 55, 12, 10, 10
    legend_h = 36
    gx0 = graph_bbox[0] + margin_left
    gy0 = graph_bbox[1] + margin_top
    gy1 = graph_bbox[3] - margin_bottom - legend_h
    gw = (graph_bbox[2] - graph_bbox[0]) - margin_left - margin_right
    gh = (graph_bbox[3] - graph_bbox[1]) - margin_top - margin_bottom - legend_h
    GRAPH_GEOM = (gx0, gy1, gw, gh)

    dash_canvas.create_line(gx0, gy0, gx0, gy1, fill = "black", width = 3, tags = "dash_static")
    for val, lbl in ((0, "0"), (7000, "7k"), (14000, "14k")):
        y = gy1 - (val / DISPLAY_MAX) * gh
        dash_canvas.create_line(gx0 - 8, y, gx0, y, fill = "black", width = 2, tags = "dash_static")
        dash_canvas.create_text(gx0 - 12, y, text = lbl, anchor = "e", font = DASH_LABEL_FONT, tags = "dash_static")

    legend_y = graph_bbox[3] - legend_h / 2
    legend_x = gx0 + 10
    spacing = 110
    for sensor_idx in sorted(CHANNEL_BUFFERS):
        color = SERIES_COLORS.get(sensor_idx, "#000000")
        x = legend_x + (sensor_idx - 1) * spacing
        dash_canvas.create_rectangle(
            x,
            legend_y - 6,
            x + 18,
            legend_y + 6,
        ...
        dash_canvas.create_text(
            x + 24,
            legend_y,
            text = f"CH{sensor_idx}",
            anchor = "w",
            font = DASH_LABEL_FONT,
            tags = "dash_static",
        )

    _DASHBOARD_STATIC_READY = True


def _init_dashboard_dynamic_scene():
    """Create reusable live-graph items once."""
    global _DASH_CHANNEL_LINE_ITEMS, _DASHBOARD_DYNAMIC_READY

    if dash_canvas is None or _DASHBOARD_DYNAMIC_READY:
        return

    _DASH_CHANNEL_LINE_ITEMS = {}
    for sensor_idx in sorted(CHANNEL_BUFFERS):
        tag = CH_LINE_TAGS.get(sensor_idx, f"channel_line_{sensor_idx}")
        series_color = SERIES_COLORS.get(sensor_idx, "#000000")
        _DASH_CHANNEL_LINE_ITEMS[sensor_idx] = dash_canvas.create_line(
            0,
            0,
            0,
            0,
            ...
            fill = series_color,
            state = "hidden",
            tags = ("dash_dynamic", tag),
        )

    _DASHBOARD_DYNAMIC_READY = True


def _create_dashboard_post_pretask_static_items():
    """Create the post-pretask static min/max guide lines once."""
    global _DASH_MINMAX_STATIC_READY

    if dash_canvas is None or GRAPH_GEOM is None or _DASH_MINMAX_STATIC_READY or not MIN_MAX_DICT:
        return

    gx0, gy1, gw, gh = GRAPH_GEOM
    display_range = DISPLAY_MAX - DISPLAY_MIN
    if display_range == 0:
        display_range = 1.0

    for sensor_idx in sorted(CHANNEL_BUFFERS):
        key = f"Sensor {sensor_idx}"
        if key not in MIN_MAX_DICT:
            continue

        color = SERIES_COLORS.get(sensor_idx, "#000000")
        min_val, max_val = MIN_MAX_DICT[key]
        for suffix, value in (("min", min_val), ("max", max_val)):
            norm01 = (float(value) - DISPLAY_MIN) / display_range
            norm01 = max(0.0, min(1.0, norm01))
            y = gy1 - norm01 * gh

            dash_canvas.create_line(
                gx0,
                y,
                gx0 + gw,
                y,
                ...
                width = 2,
                dash = (4, 4),
                tags = ("dash_static_post", f"minmax_{sensor_idx}_{suffix}"),
            )
            dash_canvas.create_line(
                gx0 - 6,
                y,
                ...
                width = 2,
                dash = (3, 3),
                tags = ("dash_static_post", f"minmax_tick_{sensor_idx}_{suffix}"),
            )

    _DASH_MINMAX_STATIC_READY = True


def _get_graph_sampling_cache(n, step, gx0, gw):
    cache_key = (n, step, gx0, gw)
    cached = _DASH_GRAPH_POINT_CACHE.get(cache_key)
    ...
    denom = max(1, n - 1)
    xs = tuple(gx0 + (i / denom) * gw for i in indices)
    cached = (tuple(indices), xs)
    _DASH_GRAPH_POINT_CACHE[cache_key] = cached
    return cached


def _update_dashboard_live_graphs():
    """Refresh the three live channel graphs every frame."""
    if dash_canvas is None or GRAPH_GEOM is None:
        return
    ...
    for sensor_idx, buf in CHANNEL_BUFFERS.items():
        item_id = _DASH_CHANNEL_LINE_ITEMS.get(sensor_idx)
        if item_id is None:
            continue

        n = len(buf)
        if n < 2:
            dash_canvas.itemconfig(item_id, state = "hidden")
            continue

        pixel_w = max(1, int(gw))
        step = max(1, math.ceil(n / pixel_w))
        indices, xs = _get_graph_sampling_cache(n, step, gx0, gw)
        buf_list = list(buf)

        pts = [0.0] * (2 * len(xs))
        out = 0
        for x, sample_idx in zip(xs, indices):
            value = float(buf_list[sample_idx])
            norm01 = (value - DISPLAY_MIN) / display_range
            norm01 = max(0.0, min(1.0, norm01))

            pts[out] = x
            pts[out + 1] = gy1 - norm01 * gh
            out += 2
        ...
        dash_canvas.coords(item_id, *pts)
        dash_canvas.itemconfig(item_id, state = "normal")


def _update_dashboard_dynamic_items():
    """Refresh only dynamic dashboard items."""
    _update_dashboard_live_graphs()


###############################################################################
# Tkinter Update Loop (main + dashboard)
###############################################################################
async def tk_update_loop():
    """
    Refresh both windows continuously.
    ...
    only the dynamic items:
      ...
      - dashboard: live channel graphs
    """
    while not APP_SHUTDOWN.is_set():
        try:
            # root_update_start_ns = time.perf_counter_ns()
            _apply_main_window_dynamic_updates()
            _update_dashboard_dynamic_items()
            root.update()
            # root_update_end_ns = time.perf_counter_ns()
            # print ((root_update_end_ns - root_update_start_ns) / 1_000_000.0)
        except tk.TclError:
            APP_SHUTDOWN.set()
            break
...
async def main():
    ...
    try:
        ...
        _create_dashboard_post_pretask_static_items()
        ...
        model_path = "model_log/App2_model_0405data_ws60_for_demo_0416_2ch_4out_until_85acc.pth"
        model = UltraLightCNN1D().to(device)
        model.load_state_dict(torch.load(model_path, map_location = device))
        model.eval()

        num_sensors = len(list(getattr(params, "selected_channels", (1,))))
